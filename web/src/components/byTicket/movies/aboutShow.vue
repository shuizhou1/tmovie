      ...
      <div class="movieCategory_list">
        <ul class="movieList active attention_width">
          <li class="movieList_item" v-for=" (item, index) in attentionArr" :key="index">
            ...
            <p>
              <span class="wantedCount">{{item.wantedCount}}人想看</span>
    ...
    <div class="aboutShow_movies">
      <ul class="aboutShow_list">
        <li class="aboutShow_listItem" v-for="(item,index) in movieComingArr" :key="index">
          <div
            class="imgBox"
            v-bind:style="{backgroundImage:`url(${item.image})`,  backgroundPosition:`center center`, backgroundSize: `100% 100%`, backgroundRepeat:'no-repeat'}"
          ></div>
          <div class="messageBox">
            <div class="left">
              ...
              <p>
                <span class="wantedCount">{{ item.wantedCount}}</span>人想看 -
                <span class="category">{{item.type}}</span>
              </p>
              <p>
                <span class="movieActors">{{ item.actor1 }} / {{ item.actor2}}</span>
              </p>
            </div>
            <div class="right">
              <p>
                <a class="orderTicket">预售</a>
              </p>
            </div>
          </div>
        </li>
...
export default {
  ...
  methods: {
    getMovieList() {
      this.$http
        .get("/api/Movie/MovieComingNew.api", {
          params: {

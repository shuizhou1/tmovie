    ...
    <div class="formWraper">
      <form action="" method="post" id="form1" onsubmit="return false">
        <div>
          ...
          <input
            type="text" placeholder="2-8位汉字、数字、字母、下划线"
        ...
        <div>
          ...
          <input type="password" placeholder="6-10位字母、数字"
    ...
    methods: {
      ...
      testUserName (e) {
       ...
       if (val.length>0 && !re.test(val)) {
          $('.err_userName').show().html('! 用户名必须由2-8位中文或数字字母下划线组成')
        } else {
          $('.err_userName').hide()
        }
      ...
      testUserPsw (e) {
        ...
        const val = $(e.target).val();
        if (val.length>0 && !re.test(val)) {
          $('.err_userPsw').show().html('! 密码为6-8为数字或字母组成')
        } else {
          $('.err_userPsw').hide()
        }
      },
      ...
      confirmPsw (e) {
        ...
        if ( val1 !== val2) {
          $('.err_confirmPsw').show().html('! 两次输入的密码不一致')
        } else {
          $('.err_confirmPsw').hide()
        }
      },
      ...
      testPhoneNum (e) {
       ...
       if (val.length>0 && !re.test(val)) {
          $('.err_phoneNum').show().html('！您输入的手机号码格式有误')
        }
        else {
          $('.err_phoneNum').hide()
        }
      ...
      register () {
        const [userName, userPsw, phoneNum] = [$('#userName'), $('#userPsw'), $('#phoneNum')]
        const [val1, val2, val3] = [ userName.val(), userPsw.val(), phoneNum.val()]

        if (val1.length === 0) {
          $('.err_userName').show().html('! 用户名不能为空')
          userName.focus()
        }
        else if (val2.length === 0) {
          $('.err_userPsw').show().html('! 密码不能为空')
          userPsw.focus()
        }
       else if (val3.length === 0) {
          $('.err_phoneNum').show().html('! 手机号码不能为空')
          phoneNum.focus()
        }
        else {
          $.ajax({
            ...
            success (data, status, c) {
              if (data.message === '用户名已经注册！') {
                $('.err_userName').show().html('！该用户名已被占用').css({'left': '30%', 'bottom': '20%'})
